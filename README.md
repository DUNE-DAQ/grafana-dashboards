...
Notable examples:
 ...
 - A TPProcessor is a `subelement` as it's generated by a monitorable objects which is a child of a DAQModule (link handler). Also note that since there is at most one TPProcessor for every datalink handler, the `subelement` is only used to identify TP related link handlers, but it's the `element` that contains the unique id name. See the "TP Handler" dashboard variables for an example. 
